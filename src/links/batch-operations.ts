...
export class BatchOperations {
	...
	async batchReplaceLink(
		...
		for (const file of files) {
			try {
				...
				// Match wiki links [[oldLink]] or [[oldLink|display]]
				const wikiLinkRegex = new RegExp(`\\[\\[${this.escapeRegex(oldLink)}(\\|[^\\]]+)?\\]\\]`, 'g');
				// Match markdown links [text](oldLink)
				const mdLinkRegex = new RegExp(`\\[([^\\]]+)\\]\\(${this.escapeRegex(oldLink)}\\)`, 'g');
	...
	/**
	 * Count occurrences of a link in content
	 */
	private countLinkOccurrences(content: string, link: string): number {
		const wikiLinkRegex = new RegExp(`\\[\\[${this.escapeRegex(link)}(\\|[^\\]]+)?\\]\\]`, 'g');
		const mdLinkRegex = new RegExp(`\\[([^\\]]+)\\]\\(${this.escapeRegex(link)}\\)`, 'g');
		
		const wikiMatches = content.match(wikiLinkRegex) || [];
		const mdMatches = content.match(mdLinkRegex) || [];
		
		return wikiMatches.length + mdMatches.length;
	}
